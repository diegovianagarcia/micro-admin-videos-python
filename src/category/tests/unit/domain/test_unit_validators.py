...
class TestCategoryValidatorUnit(unittest.TestCase):
    ...
    def test_invaliation_cases_for_name_field(self):
        invalid_data = [
        ...
        for item in invalid_data:
            is_valid = self.validator.validate(item['data'])
            self.assertFalse(is_valid)
            self.assertIn('name', self.validator.errors)
            self.assertListEqual(
                self.validator.errors['name'],
                [item['expected']],
                msg=f'Expected: {item["expected"]}, actual: {self.validator.errors["name"][0]}')

    def test_invaliation_cases_for_description_field(self):
        is_valid = self.validator.validate({'description': 5})
        self.assertFalse(is_valid)
        self.assertListEqual(self.validator.errors['description'], [
                             'Not a valid string.'])

    def test_invaliation_cases_for_is_active_field(self):
    ...
    def test_invaliation_cases_for_create_at_field(self):
    ...
    def test_validate_cases(self):
        ...
        for item in valid_data:
            is_valid = self.validator.validate(data=item)
            self.assertTrue(is_valid, msg=f'data: {item}')
