...
class TestRepositoryInterfaceUnit(unittest.TestCase):
    ...
    def test_throw_error_when_methods_not_implemented(self):
        ...
        with self.assertRaises(TypeError) as assert_error:
            RepositoryInterface()
        self.assertEqual(
            assert_error.exception.args[0], "Can't instantiate abstract class RepositoryInterface"
...
class TestInMemoryRespositoryUnit(unittest.TestCase):
    repo: StubInMemoryRepository

    def setUp(self):
        self.repo = StubInMemoryRepository()

    def testitem_prop_is_empty_on_init(self):
    ...
    def test_raise_not_found_exception_in_find_by_id(self):
        ...
        self.assertEqual(
            ...
            "Entity not found using ID '0f42ac99-08b0-4fef-923d-9187b3762a0d'")
    ...
    def test_raise_not_found_exception_in_update(self):
        entity = StubEntity(name='test', price=5)
        with self.assertRaises(NotFoundExeption) as assert_error:
    ...
    def test_raise_not_found_exception_in_delete(self):
        entity = StubEntity(name='test', price=5)
        with self.assertRaises(NotFoundExeption) as assert_error:
...
class TestSearchableRepositoryInterface(unittest.TestCase):
    ...
    def test_searchable_fields_prop(self):
...
class TestSearchParams(unittest.TestCase):
    ...
    def test_per_page_prop(self):
        ...
        for item in arrange:
            ...
            self.assertEqual(
                params.per_page, item['expected'],
                msg=f"per_page = {item['per_page']}")
    ...
    def test_sort_prop(self):
        ...
        for item in arrange:
            ...
            self.assertEqual(
                params.sort, item['expected'],
                msg=f"sort = {item['sort']}")
    ...
    def test_sort_dir_prop(self):
        ...
        for item in arrange:
            ...
            self.assertEqual(
                params.sort_dir, item['expected'],
                msg=f"sort_dir = {item['sort_dir']}")
    ...
    def test_filter_prop(self):
        ...
        for item in arrange:
            ...
            self.assertEqual(
                params.filter, item['expected'],
                msg=f"filter = {item['filter']}")
...
class StubInMemorySearchableRepository(InMemorySearchRepository[StubEntity, str]):
    sortable_fields = ['name']

    def _apply_filter(self, items: List[StubEntity], filter_param: str) -> List[ET]:
        if filter_param:
            return list(filter(lambda i: filter_param.lower() in i.name.lower()
